...
public class RerollScroll {
	...
	public boolean durabilityPlus(ItemStack is) {
		try {
			ItemMeta item = is.getItemMeta();
			ArrayList<String> lores = (ArrayList<String>) item.getLore();
			int lineNum = -1;
			...
			if(lineNum != -1) {
				...
				if(lineNum != -1) {
					lores.add(lineNum, " ");
					lores.add(lineNum, ChatColor.DARK_GRAY + "내구도: 1000/3000");
					item.setLore(lores);
					is.setItemMeta(item);
					return true;
				} else {
					lores.add(" ");
					lores.add(ChatColor.DARK_GRAY + "내구도: 1000/3000");
					item.setLore(lores);
					is.setItemMeta(item);
					return true;
				}
