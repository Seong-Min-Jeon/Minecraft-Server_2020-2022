...
public class ShootArrow {
	...
	public ShootArrow(Entity ent) {
		...
		if(arrow1(arrow, mob, loc) && arrow2(arrow, mob, loc) && arrow3(arrow, mob, loc) && arrow4(arrow, mob, loc) 
				...
				&& arrow25(arrow, mob, loc) && arrow26(arrow, mob, loc) && arrow27(arrow, mob, loc) && arrow28(arrow, mob, loc)) {
	...
	public boolean arrow20(Arrow arrow, Entity mob, Location loc) {
		if(mob.getCustomName().substring(2).equalsIgnoreCase("오징어 마법사" + ChatColor.YELLOW + " [Lv.700]")) {
			if (arrow.getPassenger() != null) {
				arrow.removePassenger(arrow.getPassenger());
			}
			Item item = arrow.getWorld().dropItem(loc, new ItemStack(Material.SALMON));
			item.setPickupDelay(10000000);
			arrow.addPassenger(item);
			return false;
		}
	...
	public boolean arrow21(Arrow arrow, Entity mob, Location loc) {
	...
	public boolean arrow25(Arrow arrow, Entity mob, Location loc) {
		if(mob.getCustomName().substring(2).equalsIgnoreCase("탑의 추종자" + ChatColor.YELLOW + " [Lv.1]")) {
			if (arrow.getPassenger() != null) {
				arrow.removePassenger(arrow.getPassenger());
			}
			Item item = arrow.getWorld().dropItem(loc, new ItemStack(Material.HEART_OF_THE_SEA));
	...
	public boolean arrow28(Arrow arrow, Entity mob, Location loc) {
